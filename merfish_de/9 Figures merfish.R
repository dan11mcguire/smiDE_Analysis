...
results_all <- read.csv(file = paste0(dir,"Results_realdata_Microglia100_all.csv"))
overlap_metrics_allen <- read.csv(paste0(dir, "Cont_pre_de_WholeBrain_distance.csv"))


# Figures -----------------------------------------------------------------
# Neurons and Microglia ---------------------------------------------------
# See 1 "Organizing data merfish.R" to get WholeBrain_seurat.Rdata
load( paste0(dir, "WholeBrain_seurat.Rdata"))
...
library(nicheDE)
dummy_class <- model.matrix(~class-1, data = dataDE2@meta.data)
colnames(dummy_class) <- gsub("class","",colnames(dummy_class))

# Get average expression profile
avg_expr <- CreateLibraryMatrix(t(dataDE2@assays$RNA$counts),
                                dataDE2@meta.data[,c("cell_ID","class")])
# NicheDE object
NDE_obj <- CreateNicheDEObject(counts_mat = t(dataDE2@assays$RNA$counts),
                               coordinate_mat = dataDE2@meta.data[,c("x","y")],
...
dataDE2@meta.data <- cbind(dataDE2@meta.data,effecniche500,effecniche1000)
# Filtering to neurons
dataDE <- subset(dataDE2, subset = class == "01 IT-ET Glut")
...
ring <- circle_df(cx, cy, 1000/4542.031) #4542.031 = NDE_obj$result@scale from load(file = paste0(dir,"ResultsDistance/NicheDE/nicheDE_Microglia_1000_negbin",".Rdata"))
...
plot_volcano <- results_all %>% filter(Method == "smiDE Naive") %>%
  ...
  ggplot(aes(x = Estimate, y = -log10(padj), col = label, pch = (label ==  "SPDE DE genes (n=3)"))) +
  geom_point(cex=1) +
...
# Priors ------------------------------------------------------------------
# SPDE ------------------------------------------------------------------
avg_neurons <- overlap_metrics_allen %>%
  filter(class == "01 IT-ET Glut") %>%
  select(target, avg_cluster)

results_all %>% 
  filter(Method == "SPDE",
      Distribution %in% c("Negative Binomial","Gaussian"), 
      meshscenario == 0.5,
                           !is.na(padj)) %>%
...
results_inla_priors <- results_all %>% 
  filter(Method == "SPDE",
         Distribution %in% c("Negative Binomial","Gaussian"), 
         meshscenario == 0.5,
         !is.na(padj)) %>%
  left_join(avg_neurons, by = c("Gene"="target")) %>%
...
results_all %>% filter(Method == "BYM2",
                           !is.na(padj)) %>%
...
results_bym2_priors <- results_all %>% 
  filter(Method == "BYM2",kprop %in% c(0.250)) %>%
  left_join(avg_neurons, by = c("Gene"="target")) %>%
